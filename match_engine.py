...
def find_matches(users_df, threshold=0.6):
    ...
    # Update users.csv with IsMatched flag
    users_df["IsMatched"] = users_df["name"].apply(
        lambda x: True if x in matched_learners else False if x in unmatched_learners else users_df.loc[users_df["name"] == x, "IsMatched"].iloc[0] if "IsMatched" in users_df.columns else False
    )
